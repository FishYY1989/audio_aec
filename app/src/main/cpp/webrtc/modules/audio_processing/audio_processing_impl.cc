...
AudioProcessingImpl::AudioProcessingImpl(const Config& config,
  ...
  m_pFileAfterHP = new TestFileWriter("audioAfterHP");
  m_pFileAfterAEC = new TestFileWriter("audioAfterAEC.pcm");
  m_pFileAfterAGC = new TestFileWriter("audioAfterECM.pcm");
  m_pFileAfterNS = new TestFileWriter("audioAfterNS.pcm");
  m_pFileAfterVAD = new TestFileWriter("audioAfterVAD.pcm");
}
...
#endif
  SAFE_DELETE(m_pFileAfterAEC)
  SAFE_DELETE(m_pFileAfterAGC)
  SAFE_DELETE(m_pFileAfterNS)
  SAFE_DELETE(m_pFileAfterVAD)
  SAFE_DELETE(m_pFileAfterHP)
}
...
int AudioProcessingImpl::ProcessStreamLocked() {
  ...
  this->saveAudioDataToFile(m_pFileAfterHP, ca);
  ...
  RETURN_ON_ERR(public_submodules_->echo_cancellation->ProcessCaptureAudio(
      ca, stream_delay_ms()));
  this->saveAudioDataToFile(m_pFileAfterAEC, ca);
  ...
  this->saveAudioDataToFile(m_pFileAfterNS, ca);
  ...
  this->saveAudioDataToFile(m_pFileAfterVAD, ca);
  ...
  this->saveAudioDataToFile(m_pFileAfterAGC,ca);
...
void AudioProcessingImpl::saveAudioDataToFile(TestFileWriter* file, AudioBuffer* audio)
{
    for (int i=0; i<audio->num_channels(); i++) {
        file->write((char*)(audio->split_bands_const(i)[kBand0To8kHz]), audio->num_frames_per_band()*2);
    }
}
