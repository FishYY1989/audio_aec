...
bool Win32Filesystem::GetAppDataFolder(Pathname* path, bool per_user) {
  ...
  if (!IsCurrentProcessLowIntegrity() &&
      !::GetLongPathName(buffer, buffer, arraysize(buffer)))
    return false;
#if defined(_MSC_VER) //VS2013 (err-C2782) -- by Yaphet
  TCHAR tmpBuf[MAX_PATH];
  memcpy(tmpBuf, __T("\\"), sizeof(__T("\\")));
  size_t len = strcatn(buffer, arraysize(buffer), tmpBuf);
#else
  ...
  if ((len > 0) && (buffer[len-1] != __T('\\'))) {
	  ...
	  TCHAR tmpBuf[MAX_PATH];
	  memcpy(tmpBuf, __T("\\"), sizeof(__T("\\")));
	  len += strcpyn(buffer + len, arraysize(buffer) - len, tmpBuf);
#else
